...
def shutdown():
    #turn off flag to get frame processor to output frames
    rospy.set_param('/planner/show_feature_view', False)

if __name__ == '__main__':
    #start subscriber
    rospy.init_node('picam_stream', anonymous=False)
    rospy.set_param('/planner/show_feature_view', True)
    HEIGHT = rospy.get_param('picam/height')
    WIDTH = rospy.get_param('picam/width')
    FRAMERATE = rospy.get_param('picam/framerate')
    rospy.Subscriber('/planner/feature_view', CamData, sub_callback)
    #start webserver
